	...
	<div class="row">	
		<div class="col-xs-12 col-md-10">
			<div class="panel panel-default">
				<a href="/library" class="btn btn-primary btn-xs pull-right m-t m-r">
					<span class="fa fa-arrow-circle-left"></span>&nbsp;Back to Library</a>
                <h4 class="p-title">Edit Media {{ $media->media_name }}</h4>
								  ...
								  action="{{ url('/edit_media') }}"> {{ method_field('PATCH') }}
									...
									<div class="form-group">
										...
										<select class="form-control m-b" id="image_category_id"
											...
											<option value="{{ $category->id }}" {{ $media->category == $category->id ? 'selected="selected"' : '' }}>{{ $category->category }}</option>
											@endforeach
										</select>
										<label class="error hide"
									...
									<div class="form-group">
										...
										<select class="form-control m-b"
											...
											<option value="{{ $locationType->id }}" {{ $media->location_type == $locationType->id ? 'selected="selected"' : '' }}>{{ $locationType->width . 'x' . $locationType->height . ' ' . $locationType->description }}</option>
											@endforeach
										</select>
